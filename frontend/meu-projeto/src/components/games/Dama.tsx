...
// Funções auxiliares de coordenada
...
export default function Dama() {
  // ✅ CORREÇÃO 1: Hooks declarados DENTRO do componente
  ...
  // ✅ CORREÇÃO 2: Código real dentro da função turnoIA
  async function turnoIA(currentBoard: BoardArray) {
    setIsThinking(true); 
    const bToSend = currentBoard.slice();
    
    try {
      const resp = await fetch("http://localhost:5000/dama/jogar", {
        method: "POST",
            ...
            <div className="inline-block bg-[#0a1628] px-6 py-2 rounded-xl border-2 border-orange-500">
              <p className="text-2xl font-bold flex items-center gap-2">
                {turno === "vermelha" 
                  ? "🔴 Seu Turno" 
                  : isThinking 
                    ? <span className="animate-pulse">⚫ IA Pensando...</span> 
                    : "⚫ Turno da IA"
                }
              </p>
            </div>
